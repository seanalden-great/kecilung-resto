...
<template>
  <div class="container mx-auto px-6 py-16 max-w-5xl">
    ...
    <div v-else-if="momentData" class="bg-white rounded-2xl shadow-lg p-8 md:flex gap-10 mb-12">
    ...
    <!-- ========================================== -->
    <div v-if="momentData" class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
      <h2 class="text-2xl font-bold text-gray-800 mb-2">Jadwal yang Sudah Terisi</h2>
      <p class="text-gray-500 mb-6">Paket moment ini telah di-booking sebanyak <strong>{{ approvedBookings.length }} kali</strong>. Harap perhatikan waktu di bawah agar jadwal reservasi Anda tidak bertabrakan.</p>
      
      <div v-if="approvedBookings.length === 0" class="bg-orange-50 text-orange-700 p-4 rounded-lg text-center font-medium">
        Belum ada jadwal yang terisi. Jadilah yang pertama reservasi!
      </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div class="bg-white p-6 md:p-8 rounded-xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        <h2 class="text-2xl font-bold mb-6 text-gray-800 border-b pb-2">Formulir Reservasi Lengkap</h2>
        <form @submit.prevent="submitBooking">
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            ...
            <div>
              ...
              <input v-model="form.description" type="text" placeholder="Cth: Dinner Ultah, Rapat" required class="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-orange-500" />
            </div>
          </div>
          ...
          <div class="flex gap-4">
            ...
            <button type="submit" :disabled="isSubmitting" class="w-2/3 bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-lg font-bold transition disabled:opacity-50">
              {{ isSubmitting ? 'Mengecek Jadwal...' : 'Ajukan Reservasi' }}
            </button>
          </div>
        </form>
      </div>
...
const { data: approvedRes, refresh: refreshBookings } = useFetch(`${baseURL}/moments/packages/${route.params.id}/bookings`, {
  lazy: import.meta.client
});
...
const form = ref({ 
  ...
  booking_date: "", booking_end_date: "", moment_id: parseInt(route.params.id) 
});
...
const submitBooking = async () => {
  ...
  try {
    ...
    showModal.value = false;
    Swal.fire('Berhasil!', 'Reservasi Anda telah diajukan tanpa tabrakan jadwal. Admin kami akan segera menghubungi Anda melalui WhatsApp untuk konfirmasi.', 'success');
    ...
    Swal.fire('Jadwal Tidak Tersedia', err.response?._data?.error || 'Gagal melakukan reservasi.', 'error');
  } finally {
    isSubmitting.value = false;
  }
};
